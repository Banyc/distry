...
impl WriteAheadLog {
    pub async fn overwrite(path: impl AsRef<Path>, base_seq: SeqNum) -> io::Result<Self> {
        ...
        match res {
            Ok(this) => Ok(this),
            Err(e) => match e {
                RebuildError::File(error) => Err(error),
                RebuildError::User(_) => unreachable!(),
            },
        }
    }
    pub async fn load_no_err(
    ...
    ) -> Result<Self, RebuildError<Infallible>> {
        ...
        match res {
            Ok(this) => Ok(this),
            Err(e) => match e {
                RebuildError::File(error) => Err(RebuildError::File(error)),
                RebuildError::User(_) => unreachable!(),
            },
        }
    }
    ...
    async fn load_or_overwrite<E>(
        ...
        loop {
            ...
            offset += n;
            read_entries(seq, &buf).map_err(RebuildError::User)?;
            seq += 1;
        }
        let file_len = u64::try_from(*offsets.last().unwrap()).unwrap();
        file.set_len(file_len).await.map_err(RebuildError::File)?;
